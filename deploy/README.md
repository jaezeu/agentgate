...
1. `deploy/bootstrap` — S3 state backend, KMS, and GitHub OIDC deployment
   trust (local state, applied once per account)
2. `deploy/infra`
3. `deploy/platform`
4. `deploy/agentgate`

The sandbox roots store state in the bootstrap-created S3 bucket with native
lock files and can be applied from an operator AWS SSO session or through the
